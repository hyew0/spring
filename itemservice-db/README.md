...
- mybatis-spring-boot-starter 라이브러리를 사용하면 MyBatis를 스프링과 통합하고, 설정도 아주 간단히 할 수 있다.
  - build.gradle에 의존 관계를 추가하여 라이브러리를 사용하도록 한다.
  - ```groovy
    //MyBatis 추가
    implementation 'org.mybatis.spring.boot:mybatis-spring-boot-starter:3.0.3'
    ```
    - MyBatis 에 버전 정보가 붙는 이유는 스프링 부트가 버전을 관리해주는 공식 라이브러리가 아니기 때문이다.
      - 스프링 부트가 버전을 관리해주는 경우에는 버전 정보를 붙이지 않아도 최적의 버전을 자동으로 찾아준다.
...
### 설정
- application.properties 에 다음 설정을 추가. #MyBatis 를 참고하면 된다.
- 주의! 
...
- mybatis.type-aliases-package
  - MyBatis 에서 타입 정보를 사용할 때는 패키지 이름을 적어주어야 하는데, 여기에 명시하면 패키지 이름을 생략할 수 있다.
...
### insert SQL
- id 에는 Mapper 인터페이스에 설정한 메서드 이름을 지정하면 된다.
...
- ItemMapper 매퍼 인터페이스의 구현체가 없는데 어떻게 동작한 것일까?
  - MyBatis 스프링 연동 모듈에서 자동으로 처리해준다.
    - 1.@Mapper 조회 (MyBatis 스프링 연동 모듈 -> ItemMapper interface)
      - 애플리케이션 로딩 시점에 MyBatis 스프링 연동 모듈은 @Mapper 가 붙어있는 인터페이스를 조사한다.
    - 2.동적 프록시 객체 생성 (MyBatis 스프링 연동 모듈 -> ItemMapper 구현체 proxy)
      - 해당 인터페이스(ItemMapper)가 발견되면 동적 프록시 기술을 사용해서 ItemMapper 인터페이스의 구현체를 만든다.
    - 3.생성된 동적 프록시 객체를 스프링빈으로 등록 (MyBatis 스프링 연동 모듈 -> 스프링 컨테이너)
...
```
- 위의 예제의 문제점은 
  - 모든조건을 만족하지 못하거나 가장 앞의 값이 만족하지 못할 경우에 Where 만 들어가고 값이 들어가지 않거나 and가 가장 앞에 들어가는 문제가 발생한다.
  - 이 문제점은 <where>로 해결 가능하다.
- <where>는 문장이 없으면 where 를 추가하지 않는다. 
  - 문장이 있으면 where 를 추가한다. 
  - 만약 and 가 먼저 시작된다면 and 를 지운다.

- trim 이라는 기능으로 사용해도 된다. 
...
- 때로는 파라미터 바인딩이 아니라 문자 그대로를 처리하고 싶은 경우도 있다. 
  ...
  - ```
    ...
    - 주의!
      ...
      - 따라서 가급적 사용하면 안된다. 사용할거라면 주의깊게 사용해야 한다.
